...
def main(args):
    ...
    for f in demo_files:
        ...
        file_name = f.split('.')[0]
        demo_recon = np.load(demo_path + f'/{file_name}_recon.npz', allow_pickle=True)
        demos.append(demo)
        demos_recon.append(demo_recon)
    ...
    print(
        f'Parent model name specific: {parent_model_name_specific}, Child model name specific: {child_model_name_specific}, EBM name specific: {ebm_model_name_specific}')
    ...
    pb_client = create_pybullet_client(gui=args.pybullet_viz, opengl_render=True, realtime=True,
    ...
    if args.recon and args.model == "mimo":
        config = load_dataclass(InferenceConfig, 'config/demo_recon.yaml')
        dev = torch.device('cuda:0')
        mesh_recon_master = MeshReconstruction(model=child_model, device=dev, cfg=config)
        config.padding = 0.05
        if args.parent_class == 'mug':
            config.threshold = 0.65
        else:
            config.threshold = 0.7
        mesh_recon_slave = MeshReconstruction(model=parent_model, device=dev, cfg=config)
    ...
    pc_master_dict['parent']['test_ids'] = np.loadtxt(
        osp.join(path_util.get_rndf_share(), '%s_test_object_split.txt' % parent_class), dtype=str).tolist()
    pc_master_dict['child']['test_ids'] = np.loadtxt(
        osp.join(path_util.get_rndf_share(), '%s_test_object_split.txt' % child_class), dtype=str).tolist()
    ...
    for pc in pcl:
        ...
        if pc_master_dict[pc]['class'] == 'syn_container':
            pc_master_dict[pc]['scale_hl'] = [1.1, 0.9]
            pc_master_dict[pc]['scale_default'] = 1.0
            # pc_master_dict[pc]['scale_hl'] = [1.3, 1.0]
            # pc_master_dict[pc]['scale_default'] = 1.15
    ...
    pc_master_dict['parent']['demo_final_recon_pcds'] = []
    pc_master_dict['child']['demo_start_pcds'] = []
    pc_master_dict['child']['demo_final_pcds'] = []
    pc_master_dict['child']['demo_final_recon_pcds'] = []
    for pc in pcl:
        ...
        for idx, demo in enumerate(demos_recon):
            recon_f_pcd = demo[f'pcd_{pc}']

            if pc == "child":
                recon_f_pcd += np.array([0, 0, 1 / 256])

            pc_master_dict[pc]['demo_final_recon_pcds'].append(recon_f_pcd)
    ...
    if args.relation_method == 'intersection':
        if not osp.exists(target_desc_fname) or args.new_descriptors:
            ...
            if parent_class == 'syn_container' and child_class == 'bottle':
                ...
                keypoint_offset_params = {'offset': 0.00, 'type': 'bottom'}
            else:
                use_keypoint_offset = False
                keypoint_offset_params = None

            if args.recon and args.model in ["occsdfscf", "mfim"]:
                recon = True
            else:
                recon = False
            create_target_descriptors(
                ...
                visualize=True, mc_vis=mc_vis, recon=recon)
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        if parent_class == 'syn_container' and child_class == 'bottle':
            ...
            container_upright_mat = np.eye(4);
            container_upright_mat[:-1, :-1] = common.quat2rot(container_upright_orientation)
            bottle_upright_mat = np.eye(4);
            bottle_upright_mat[:-1, :-1] = common.quat2rot(bottle_upright_orientation)
            ...
            with recorder.meshcat_scene_lock:
                util.meshcat_trimesh_show(mc_vis, 'scene/container_box',
                                          container_box.to_mesh().apply_translation([0.0, 0.2, 0.0]), color=(255, 0, 0))
                util.meshcat_trimesh_show(mc_vis, 'scene/bottle_box',
                                          bottle_box.to_mesh().apply_translation([0.0, -0.2, 0.0]), color=(0, 0, 255))
            ...
            if np.max(bottle_extents) > (0.75 * np.min(container_extents[:-1])):
                ...
                new_parent_scale = np.max(bottle_extents) * (np.random.random() * (2 - 1.5) + 1.5) / np.min(
                    container_extents[:-1])
        ...
        for pc in pcl:
            ...
            if load_pose_type == 'any_pose':
                ...
                if load_pose_type == 'demo_pose':
                    ...
                    pos = [np.random.random() * (x_high - x_low) + x_low, np.random.random() * (y_high - y_low) + y_low,
                           table_z]
            ...
            safeCollisionFilterPair(bodyUniqueIdA=obj_id, bodyUniqueIdB=table_id, linkIndexA=-1,
                                    linkIndexB=table_base_id, enableCollision=False)
        ...
        obj_id = pc_master_dict['child']['pb_obj_id']
        if parent_class == 'mug' and child_class == 'bowl':
            obj_id = pc_master_dict['parent']['pb_obj_id']
            ...
            x = - pose_mat[0, 2]
            y = - pose_mat[1, 2]
        elif parent_class == 'syn_rack_easy' and child_class == 'mug':
            obj_id = pc_master_dict['child']['pb_obj_id']
            obj_pose_world = p.getBasePositionAndOrientation(obj_id)
            obj_pose_world = util.list2pose_stamped(list(obj_pose_world[0]) + list(obj_pose_world[1]))
            pose_mat = matrix_from_pose(obj_pose_world)
        ...
        if args.recon and args.model in ["occsdfscf", "mfim"]:
            ...
            child_pcd = child_pcd[:1500]
            pcd_mean = np.mean(child_pcd, axis=0)
            pcd = child_pcd - pcd_mean
            mesh_master = mesh_recon_master.from_occupancy(pcd)
            # pts = mesh_master.sample(2000)
            # child_pcd = pts + pcd_mean

            # postprocess
            mesh = mesh_master.as_open3d
            ...
            child_pcd = recon_pts + pcd_mean

            np.random.shuffle(parent_pcd)
            parent_pcd = parent_pcd[:1500]
            pcd_mean = np.mean(parent_pcd, axis=0)
            pcd = parent_pcd - pcd_mean
            mesh_slave = mesh_recon_slave.from_occupancy(pcd)
            # pts = mesh_slave.sample(2000)
            # parent_pcd = pts + pcd_mean

            # postprocess
            mesh = mesh_slave.as_open3d
            triangle_clusters, cluster_n_triangles, cluster_area = (mesh.cluster_connected_triangles())
            triangle_clusters = np.asarray(triangle_clusters)
            cluster_n_triangles = np.asarray(cluster_n_triangles)
            ...
            #
            # ps.register_point_cloud(f"master", child_pcd, enabled=True)
            # ps.register_point_cloud(f"slave", parent_pcd, enabled=True)
            # ps.register_point_cloud(f"demo_master", pc_obs_info['pcd']['parent'] + np.array([0.3, 0, 0]), enabled=True)
            # ps.register_point_cloud(f"demo_slave", pc_obs_info['pcd']['child'] + np.array([0.3, 0, 0]), enabled=True)
            #
            # ps.show()
        ...
        safeCollisionFilterPair(pc_master_dict['child']['pb_obj_id'], table_id, -1, table_base_id,
        ...
        obj_surf_contacts = p.getContactPoints(pc_master_dict['child']['pb_obj_id'],
                                               pc_master_dict['parent']['pb_obj_id'], -1, -1)
        touching_surf = len(obj_surf_contacts) > 0
        success_crit_dict['touching_surf'] = touching_surf
        if parent_class == 'syn_container' and child_class == 'bottle':
            bottle_final_pose = np.concatenate(
                p.getBasePositionAndOrientation(pc_master_dict['child']['pb_obj_id'])[:2]).tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--parent_load_pose_type', type=str, default='demo_pose',
                        help='Must be in [any_pose, demo_pose, random_upright]')
    parser.add_argument('--child_load_pose_type', type=str, default='demo_pose',
                        help='Must be in [any_pose, demo_pose, random_upright]')
    ...
    parser.add_argument('--create_target_desc', action='store_true',
