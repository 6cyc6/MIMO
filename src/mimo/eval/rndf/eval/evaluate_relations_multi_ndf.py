...
from rndf_robot.eval.demo_recon import InferenceConfig
...
from rndf_robot.utils.util import matrix_from_pose
...
def main(args):
    ...
    o_dim = 9
    if args.model == "occsdfscf":
        parent_model = VNNOccSdfScfNetMulti(latent_dim=256, model_type='pointnet', o_dim=5, return_features=True,
                                            sigmoid=True).cuda()
        child_model = VNNOccSdfScfNetMulti(latent_dim=256, model_type='pointnet', o_dim=o_dim, return_features=True,
                                           sigmoid=True).cuda()
    elif args.model == "mfim":
    ...
    if args.recon and args.model in ["mfim", "occsdfscf"]:
        config = load_dataclass(InferenceConfig, 'demo_recon.yaml')
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        relative_trans = infer_relation_intersection(
            ...
            parent_pcd, child_pcd, parent_query_points, child_query_points, opt_visualize=args.opt_visualize, offset=offset)
        pause_mc_thread(False)
