...
from mimo.eval.ndf.share.globals import bad_shapenet_mug_ids_list, bad_shapenet_bowls_ids_list, bad_shapenet_bottles_ids_list
from mimo.eval.ndf.utils.franka_ik import FrankaIK
from mimo.eval.ndf.utils.eval_gen_utils import (
...
def main(args, global_dict):
    ...
    if args.dgcnn:
        if args.model == 'occ':
            ...
            model = VNNMIMOShared(latent_dim=256, model_type='dgcnn', o_dim=16, return_features=True).cuda()
    else:
        if args.model == 'occ':
            model = VNNOccNet(
                latent_dim=256,
            ...
            model = VNNMIMOShared(latent_dim=256, model_type='pointnet', o_dim=16, return_features=True).cuda()
    ...
    for i, fname in enumerate(grasp_demo_filenames):
        ...
        if args.recon:
            if args.ibs:
                ...
                recon_pcd = place_ibs_data["recon"] - np.array([0, 0, 0.125])
    ...
    while iteration < args.num_iterations:
        ...
        # obj_shapenet_id = test_shapenet_ids[iteration]
        ...
        if target_obj_pcd_obs.shape[0] < 1001:
            log_warn("Not enough points for point cloud, skip.")
        ...
        # optimize grasp pose
        pre_grasp_ee_pose_mats, best_idx = grasp_optimizer.optimize_transform_implicit(target_obj_pcd_obs, ee=True, m=args.model)
        ...
        rack_pose_mats, best_rack_idx = place_optimizer.optimize_transform_implicit(target_obj_pcd_obs, ee=False, m=args.model)
        ...
        for g_idx in range(2):
            ...
            for i in range(p.getNumJoints(robot.arm.robot_id)):
                ...
                safeCollisionFilterPair(robot.arm.robot_id, table_id, i, placement_link_id, enableCollision=False)
