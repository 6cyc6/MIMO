...
class VNNScfNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderInner(dim=3, z_dim=latent_dim, c_dim=0, o_dim=5, hidden_size=latent_dim,
    ...
    def forward(self, input):
        ...
        if self.return_features:
            out_dict['scf'], out_dict['features'] = self.decoder(query_points, z)
        else:
            out_dict['scf'] = self.decoder(query_points, z)
    ...
    def forward_latent(self, z, coords, return_dict=False):
        ...
        out_dict['scf'], out_dict['features'] = self.decoder(coords, z)
...
class VNNOccNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderInner(dim=3, z_dim=latent_dim, c_dim=0, o_dim=1, hidden_size=latent_dim, leaky=True,
                                    sigmoid=sigmoid, return_features=return_features, acts=acts)
...
class VNNSdfNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderInner(dim=3, z_dim=latent_dim, c_dim=0, o_dim=1, hidden_size=latent_dim,
                                    leaky=True, sigmoid=False, return_features=return_features, acts=acts)
    ...
    def forward(self, input):
        ...
        if self.return_features:
            out_dict['sdf'], out_dict['features'] = self.decoder(query_points, z)
        else:
            out_dict['sdf'] = self.decoder(query_points, z)
    ...
    def forward_latent(self, z, coords, return_dict=False):
        out_dict = {}
        coords = coords * self.scaling
        out_dict['sdf'], out_dict['features'] = self.decoder(coords, z)
